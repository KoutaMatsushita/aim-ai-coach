...
export const getKovaaksStatsByDiscordId = createTool({
	...
	execute: async ({ context, runtimeContext }) => {
		...
		const medianAccuracy = sortedAccuracy[Math.floor(sortedAccuracy.length / 2)];

		// Consistency Index (1 - CV where CV = std/mean)
		const stdAccuracy = Math.sqrt(
			accuracies.reduce((acc, val) => acc + (val - avgAccuracy) ** 2, 0) / accuracies.length
		);
...
export const getAimlabStatsByDiscordId = createTool({
	...
	execute: async ({ context, runtimeContext }) => {
		...
		const stdScore = Math.sqrt(
			scores.reduce((acc, val) => acc + (val - avgScore) ** 2, 0) / scores.length
		);
		const consistencyIndex = avgScore > 0 ? Math.max(0, 1 - stdScore / avgScore) : 0;
		...
		const diff = secondHalfAvg - firstHalfAvg;
		const trend =
			diff > avgScore * 0.05 ? "improving" : diff < -avgScore * 0.05 ? "declining" : "stable";
