...
export const knowledgesApp = new Hono<{
    ...
    .post(
        ...
        async (c) => {
            ...
            try {
                const ragLibSQLService = new RAGLibSQLService(c.var.mastra.getVector("vector"))
                await ragLibSQLService.initializeIndex();

                // YouTube URLからvideo IDを抽出
                const videoId = extractVideoId(url);
                if (!videoId) return c.json({ success: false, error: "video id not found" }, 400)

                // 動画の詳細情報を取得
                const videoDetails = await youtubeService.getVideoDetails([videoId]);
                if (videoDetails.length === 0) return c.json({ success: false, error: "video not found" }, 400)
