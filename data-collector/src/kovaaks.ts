...
import { localCompleteKovaaksScore } from "../../lib/db";
...
export const uploadKovaaks = async (path: string, mastraClient: MastraClient, user: User) => {
	try {
		...
		await db.insert(localCompleteKovaaksScore).values(processedFileHashes).onConflictDoNothing();
...
const parseKovaaks = async (path: string, file: string, user: User) => {
	try {
		...
		if (lines.length < 2) {
			logger.warn("File has insufficient data (needs header + at least 1 data row)", {
				file,
...
const mapKovaaksRow = (raws: { [p: string]: any }[], meta: MetaData, user: User) => {
	return raws.map((raw) => {
		try {
			return {
				userId: user.id,
